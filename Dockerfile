...
# Using bookworm for better ARM64 support
...
# 0 – Set timezone & install optimized runtime dependencies for ARM
RUN apt-get update && apt-get install -y --no-install-recommends \
    tzdata \
    curl \
    libopenblas0 \
...
# 4 – Create directories (Optimized for Docker Named Volumes)
