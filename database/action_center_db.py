...
def create_pending_order(user_id, api_type, order_data):
    ...
    try:
        ...
        logger.info(f"Pending order created: ID={pending_order.id}, user={user_id}, type={api_type}, time={pending_order.created_at_ist}")
...
def approve_pending_order(order_id, approved_by, user_id):
    ...
    try:
        ...
        if pending_order:
            ...
            logger.info(f"Order approved: ID={order_id}, by={approved_by}, time={pending_order.approved_at_ist}")
            return True
        else:
...
def reject_pending_order(order_id, reason, rejected_by, user_id):
    ...
    try:
        ...
        if pending_order:
            ...
            logger.info(f"Order rejected: ID={order_id}, by={rejected_by}, time={pending_order.rejected_at_ist}, reason={reason}")
            return True
        else:
...
def update_broker_status(pending_order_id, broker_order_id, broker_status):
    ...
    try:
        ...
        if pending_order:
            ...
            logger.info(f"Broker status updated: pending_order={pending_order_id}, broker_order={broker_order_id}, status={broker_status}")
            return True
        else:
